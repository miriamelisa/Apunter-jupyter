...
######ENTRYS
        ...
        self.texto_scroll = scrolledtext.ScrolledText(self, width=70, height=10)
        self.texto_scroll.grid(row=7,column=0,columnspan=3,padx=10,pady=10)


    def borrar(self):
        self.url.set('')
        self.bsq.set('')
        self.texto_scroll.delete(1.0, tk.END)
    ...
    def guardar_datos(self):
        ...
        if url:
            ...
            if not url_existente:
                ...
                datos_h1 = [str(h1) for h1 in soup.find_all("h1")]
                datos_h2 = [str(h2) for h2 in soup.find_all("h2")]
                datos_p = [str(p) for p in soup.find_all("p")]
                datos_a = [str(a) for a in soup.find_all("a")]
                datos_address = [str(address) for address in soup.find_all("address")]
                datos_img = [str(img) for img in soup.find_all("img", src=True)]
                ...
                for enlaces in datos_a:
                    cursor.execute('INSERT INTO enlaces (url_id, enlaces) VALUES (?, ?)', (url_id, enlaces))
                ...
                messagebox.showwarning("Error", "Puede que la URL ya se encuentre registrada o debes introducir una URL válida.")
            self.url.set('')
    ...
    def buscar(self):
        ...
        cursor.execute('''
            SELECT url_id, titulo FROM titulos WHERE titulo LIKE ? UNION
            SELECT url_id, subtitulo FROM subtitulos WHERE subtitulo LIKE ? UNION
            SELECT url_id, parrafo FROM parrafos WHERE parrafo LIKE ? UNION
            SELECT url_id, enlaces FROM enlaces WHERE enlaces LIKE ? UNION
            SELECT url_id, direccion FROM direcciones WHERE direccion LIKE ? UNION
            SELECT url_id, src FROM imagenes WHERE src LIKE ?
        ''', (f'%{palabra_busqueda}%',) * 6)
        ...
        if resultados:
            ...
            messagebox.showinfo("Sin Resultados", f"No se encontraron resultados para '{palabra_busqueda}'.")

    def mostrar_busqueda(self, resultados):
        self.texto_scroll.delete(1.0, tk.END)
        palabra_busqueda = self.bsq.get()

        for url_id, texto_html in resultados:
            # Utilizar BeautifulSoup para extraer solo el texto de la etiqueta HTML
            soup = BeautifulSoup(texto_html, "html.parser")
            texto = soup.get_text()

            # Verificar si la palabra buscada está presente en el texto
            if palabra_busqueda.lower() in texto.lower():
                categoria = ""
                if texto_html.startswith("<h1"):
                    categoria = "Título"
                elif texto_html.startswith("<h2"):
                    categoria = "Subtítulo"
                elif texto_html.startswith("<p"):
                    categoria = "Párrafo"
                elif texto_html.startswith("<a"):
                    categoria = "Enlace"
                elif texto_html.startswith("<address"):
                    categoria = "Dirección"
                elif texto_html.startswith("<img"):
                    categoria = "Imagen"
                else:
                    categoria = "Texto"

                self.texto_scroll.insert(tk.END, f"\n\nURL ID: {url_id}\n{categoria}: {texto}\n")
                self.texto_scroll.insert(tk.END, f"Etiqueta: {texto_html}\n")
